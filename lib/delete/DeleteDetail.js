...
var getRowFields = function getRowFields(schema, modelName, node, nodeOrder) {
  var fieldDefinitions = schema.getFields(modelName);
  ...
  var fields = nodeOrder.map(function (key) {
    ...
    var override = R.path([modelName, 'deleteModal', 'rows', key], schema.schemaJSON);
...
var ReviewTable = function ReviewTable(_ref3) {
  ...
  if (!R.isEmpty(table)) {
    ...
    var customHeaders = R.path([nodeModelName, 'deleteModal', 'headers'], schema.schemaJSON);

    if (!customHeaders) {
      // pick fields that 'node' contains & order them by 'fieldOrder'
      var headerFields = getHeaders(schema, nodeModelName, node);
      var fieldOrder = R.propOr([], 'fieldOrder', schema.getModel(nodeModelName));
  ...
  var tableDisplayName = schema.getModelLabel({
...
var RemoveDetail = function RemoveDetail(_ref5) {
  ...
  var name = schema.getDisplayValue({
    modelName: modelName,
    node: node,
    customProps: customProps
  });
  var parentField = schema.getFieldLabel({
